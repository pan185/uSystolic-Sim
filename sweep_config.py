...
def gen_run_config(tech_node=None):
    ...
    sram_list = ['sram_16_16', 'sram_8_16', 'sram_4_16', 'sram_16_8', 'sram_8_8', 'sram_4_8', 'sram_16_32', 'sram_8_32', 'sram_4_32']
    ...
    for a in arch_list:
        for b in bit_list:
            ...
            #             cfg_log = ""
            #             cfg_file = open("./config/" + path + "/network.csv", "r")path
            #                     continue
            #                 elems = entry.strip().split(',')
            #                 elems = prune(elems)
            #                 if len(elems) == 11:
            ...
            #                     cfg_log += entry
            #             cfg_file.close()
            #             cfg_file = open("./config/" + path + "/sram.cfg", "w")pathig/" + path
            #                 os.system(cmd)
            #             else:
            #                 raise ValueError("Unknown word bit setting.")
            ...
            for c in cycle_list[:1]:
                for n in network_list:
                    for r in ram_list:
                        for m in sram_list: 
                            path = a + "_" + b.zfill(2) + "b_" + computing + "_" + c.zfill(3) + "c_" + n + "_" + r + '_' + m
                            print(path)
                            if not os.path.exists("./config/" + path):
                                os.system("mkdir ./config/" + path)
                            cmd = "cp ./config_src/network_config/" + n + "/network.csv " + "./config/" + path
                            ...
                            # get sram.cfg
                            cmd = f"cp ./config_src/memory_config/{m}.cfg " + "./config/" + path + '/sram.cfg'
                            os.system(cmd)
            ...
            for n in network_list:
                for r in ram_list:
                    for m in sram_list:
                        path = a + "_" + b.zfill(2) + "b_" + computing + "_" + "001c_" + n + "_" + r + '_' + m
                        print(path)
                        if not os.path.exists("./config/" + path):
                            os.system("mkdir ./config/" + path)
                        cmd = "cp ./config_src/network_config/" + n + "/network.csv " + "./config/" + path
